                ...
                if (logFileInfo.Exists)
                {
                    ...
                    if (timeSpan.Days > numDaysToKeep)
                    {
                        try
                        {
                            ...
                            string fileName = Path.GetFileNameWithoutExtension(logFilePath) + ".OLD.log";
                            string oldLogPath = Path.Combine(logFileInfo.Directory.FullName, fileName);
                            ...
                            File.Copy(logFilePath, oldLogPath, true);

                            // StreamWriter incorrectly set the created date
                            // For some reason StreamWriter is using the deleted file creation date
