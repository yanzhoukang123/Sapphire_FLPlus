...
namespace Azure.Image.Processing
{
    public static class ImageProcessingHelper
    {
        ...
        public static unsafe void GetPixelIntensity(WriteableBitmap srcImage, Point pt, ref int iRedData, ref int iGreenData, ref int iBlueData, ref int iGrayData, int iDynamicBit)
        {
            if (srcImage == null)
            {
                iRedData = 0;
            ...
            if (x < 0) { x = 0; }
            if (y < 0) { y = 0; }
            if (x >= imageWidth) { x = imageWidth - 1; }    //zero indexed
            if (y >= imageHeight) { y = imageHeight - 1; }  //zero indexed
            ...
            try
            {
                ...
                else if (srcImage.Format == PixelFormats.Gray16)
                {
                    ...
                    {
                        //EDR image?
                        cnvFactor = iDynamicBit / 16.0;
                        if (iRedData > 0)
                            iRedData = (int)Math.Pow(iRedData, cnvFactor);
                    }
                    iGreenData = iBlueData = iRedData;
                }
                else if (srcImage.Format == PixelFormats.Gray32Float)
                {
                    iRedData = (int)(*((float*)(pdata + y * bufferWidth + x * 4)));
                    iGreenData = iBlueData = iRedData;
                }
                else if (srcImage.Format == PixelFormats.Rgb48)
                {
                    ...
                    if (iDynamicBit > 16)
                    {
                        ...
                        if (iRedData > 0)
                            iRedData = (int)Math.Pow(iRedData, cnvFactor);
                        if (iGreenData > 0)
                            iGreenData = (int)Math.Pow(iGreenData, cnvFactor);
                        if (iBlueData > 0)
                            iBlueData = (int)Math.Pow(iBlueData, cnvFactor);
                        if (iGrayData > 0)
                            iGrayData = (int)Math.Pow(iGrayData, cnvFactor);
                    }
                }
        ...
        public static unsafe void UpdateDisplayImage(ref WriteableBitmap srcimg, ImageInfo imginfo, ref WriteableBitmap dstimg)
        {
            ...
            try
            {
                    ...
                    srcPixelFormat.BitsPerPixel == 64)
                {
                    ...
                    if (imginfo.SelectedChannel == ImageChannelType.Mix)
                    {
                        ...
                        if (imginfo.MixChannel.IsAutoChecked == true)
                        {
                            int nBlackValue = 0;
                            int nWhiteValue = 0;
                            ImageProcessing.GetAutoScaleValues(srcimg, ref nWhiteValue, ref nBlackValue, nDynamicBit,ImageChannelType.Mix);
                    ...
                    else
                    {
                        ...
                        if (imginfo.SelectedChannel == ImageChannelType.Red)
                        {
                            ...
                            if (imginfo.RedChannel.IsAutoChecked)
                            {
                                int nBlackValue = 0;
                                int nWhiteValue = 0;
                                ImageProcessing.GetAutoScaleValues(srcimg, ref nWhiteValue, ref nBlackValue, nDynamicBit, ImageChannelType.Red);
                        ...
                        else if (imginfo.SelectedChannel == ImageChannelType.Green)
                        {
                            ...
                            if (imginfo.GreenChannel.IsAutoChecked)
                            {
                                int nBlackValue = 0;
                                int nWhiteValue = 0;
                                ImageProcessing.GetAutoScaleValues(srcimg, ref nWhiteValue, ref nBlackValue, nDynamicBit, ImageChannelType.Green);
                        ...
                        else if (imginfo.SelectedChannel == ImageChannelType.Blue)
                        {
                            ...
                            if (imginfo.BlueChannel.IsAutoChecked)
                            {
                                int nBlackValue = 0;
                                int nWhiteValue = 0;
                                ImageProcessing.GetAutoScaleValues(srcimg, ref nWhiteValue, ref nBlackValue, nDynamicBit, ImageChannelType.Blue);
                        ...
                        else if (imginfo.SelectedChannel == ImageChannelType.Gray)
                        {
                            ...
                            if (imginfo.GrayChannel.IsAutoChecked)
                            {
                                int nBlackValue = 0;
                                int nWhiteValue = 0;
                                ImageProcessing.GetAutoScaleValues(srcimg, ref nWhiteValue, ref nBlackValue, nDynamicBit, ImageChannelType.Gray);
                ...
                else if (srcimg.Format.BitsPerPixel == 8 || srcimg.Format.BitsPerPixel == 16)
                {
                    ...
                    int nBlackValue = 0;
                    int nWhiteValue = 65535;
                    double dGammaValue = 1.0;

                    // Extended dynamic range image?
                    if (srcimg.Format.BitsPerPixel == 16 && nDynamicBit > 16)
                    {
                        //nWhiteValue = (1 << imginfo.DynamicBit) - 1;
                        //nWhiteValue = (int)Math.Pow(2, dynamicBit) - 1;
                        nWhiteValue = imginfo.MaxPixelValue;
                    }
                    else
                    {
                        nWhiteValue = (srcimg.Format.BitsPerPixel == 16) ? 65535 : 255;
                    }

                    if (imginfo.MixChannel.IsAutoChecked == true)
                    {
                        ImageProcessing.GetAutoScaleValues(srcimg, ref nWhiteValue, ref nBlackValue, nDynamicBit);
                        imginfo.MixChannel.BlackValue = nBlackValue;
                        imginfo.MixChannel.WhiteValue = nWhiteValue;
        ...
        public static unsafe void UpdateRotatingDisplayImage(WriteableBitmap srcimg, ref WriteableBitmap dstimg, int blackValue, int whiteValue, double gammaValue, int maxInput,
                                                             bool IsSaturationChecked, int SaturationThreshold, bool IsInvertChecked, int dynamicBit)
        {
            ...
            try
            {
                if (dstimg.Format.BitsPerPixel == 8 || dstimg.Format.BitsPerPixel == 16)
                {
                    ...
                    {
                        dstimg = ImageProcessing.Scale_16u8u_C1_Indexed(srcimg, nBlackValue, nWhiteValue, dGammaValue, maxInput,
                                                                        IsSaturationChecked,
                                                                        SaturationThreshold,
                                                                        IsInvertChecked,
                                                                        dynamicBit);
                    ...
                    {
                        ImageProcessing.Scale_16u8u_C1_Indexed(ref srcimg, ref dstimg, nBlackValue, nWhiteValue, dGammaValue, maxInput,
                                                               IsSaturationChecked,
                                                               SaturationThreshold,
                                                               IsInvertChecked,
                                                               dynamicBit);
        ...
        public static unsafe void UpdatePixelSingleMoveDisplayImage(ref WriteableBitmap srcimg, int PixelX1, int PixelX2, int PixelY1, int PixelY2)
        {
            if (srcimg == null)
            { return; }
            try
            {
                ImageProcessing.PixelSingleMove(ref srcimg, PixelX1, PixelX2, PixelY1, PixelY2);
            }
            catch (OutOfMemoryException oome)
            {
                //GC.Collect(GC.MaxGeneration, GCCollectionMode.Forced);
        ...
        public static unsafe void UpdatePixeleffectiveMoveDisplayImage(ref WriteableBitmap srcimg, int pixelOffset)
        {
            ...
            try
            {
                ImageProcessing.PixelSingleMoveCapturetheimage(ref srcimg, pixelOffset);
            }
            catch (OutOfMemoryException oome)
            {
                //GC.Collect(GC.MaxGeneration, GCCollectionMode.Forced);
        ...
        public static unsafe void YAxisUpdatePixeleffectiveMoveDisplayImage(ref WriteableBitmap srcimg, int pixelOffset)
        {
            ...
            try
            {
                ImageProcessing.YAxiePixelSingleMoveCapturetheimage(ref srcimg, pixelOffset);
            }
            catch (OutOfMemoryException oome)
            {
                //GC.Collect(GC.MaxGeneration, GCCollectionMode.Forced);
        ...
        public static unsafe void UpdatePixeleffectiveMoveDisplayImage(ref WriteableBitmap srcimg, ref WriteableBitmap dstimg, int pixelOffset)
        {
            if (srcimg == null) { return; }

            try
            {
                ImageProcessing.PixelSingleMoveCapturetheimage(ref srcimg, ref dstimg, pixelOffset);
            }
            catch (OutOfMemoryException oome)
            {
                //GC.Collect(GC.MaxGeneration, GCCollectionMode.Forced);
        ...
        public unsafe static void UpdateDisplayImage(byte*[] srcImages, int nWidth, int nHeight, int nStride,
                                                     PixelFormatType pixelFormat, ImageInfo imageInfo,
                                                     byte* pDstImageData, int nDstStride, ref bool bIsAutoProcessed)
        {
            if (srcImages == null || srcImages.Length == 0)
            {
                return;
            ...
            int nSaturationThreshold = imageInfo.SaturationThreshold;
            int channelCount = imageInfo.NumOfChannels;
            bIsAutoProcessed = false;   //Work-around for SFL image previewing auto-contrast (auto-contrast calculated?)

            try
            {
                if (channelCount > 1)
                {
                    ImageProcessing.Scale_16u8uC1_C4(srcImages, nWidth, nHeight, nStride,
                                                     pixelFormat, imageInfo,
                                                     pDstImageData, nDstStride,
                                                     bIsSaturation, nSaturationThreshold,
                                                     ref bIsAutoProcessed);
                }
            }
            catch (Exception)
            {
        ...
        public unsafe static void UpdateDisplayImage(byte* pSrcimage, int nWidth, int nHeight, int nSrcStride,
                                                     PixelFormatType srcPixelFormat, ImageInfo imageinfo,
                                                     byte* pDstImageData, int nDstStride, ref bool bIsAutoProcessed)
        {
            ...
            if (imageinfo.MixChannel.IsAutoChecked)
            {
                try
                {
                    int nCalcBlackVal = 0;
                    int nCalcWhiteVal = 0;
                    ImageProcessing.GetAutoScaleValues(pSrcimage, nWidth, nHeight, nSrcStride, srcPixelFormat, ref nCalcWhiteVal, ref nCalcBlackVal, nDynamicBit);
                    nBlackValue = nCalcBlackVal;
                    nWhiteValue = nCalcWhiteVal;
                    imageinfo.MixChannel.BlackValue = nBlackValue;
                    imageinfo.MixChannel.WhiteValue = nWhiteValue;
            ...
            if (srcPixelFormat == PixelFormatType.P16u_C1 || srcPixelFormat == PixelFormatType.P8u_C1)
            {
                //PixelFormat pixelFormat = PixelFormats.Gray16;
                //if (srcPixelFormat == PixelFormatType.P16u_C1)
                //    pixelFormat = PixelFormats.Gray16;
                //else if (srcPixelFormat == PixelFormatType.P8u_C1)
                //    pixelFormat = PixelFormats.Gray8;
                //// Use an indexed bitmap format.
                //ImageProcessing.Scale_16u8u_C1_Indexed(pSrcimage, nWidth, nHeight, nStride, pixelFormat,
                //                                       pDstImageData, nDstStride,
                //                                       nBlackValue, nWhiteValue, dGammaValue, bIsSaturation,
                //                                       nSatThreshold, bIsInverted);

                PixelFormat pixelFormat = (srcPixelFormat == PixelFormatType.P16u_C1) ? PixelFormats.Gray16 : PixelFormats.Gray8;
                /*if (nWidth * nHeight > (20000 * 20000))
                {
                    ...
                    // Use an indexed bitmap format.
                    ImageProcessing.Scale_16u8u_C1_Indexed(pSrcimage, nWidth, nHeight, nSrcStride, pixelFormat,
                                                           pDstImageData, nDstStride,
                                                           nBlackValue, nWhiteValue, dGammaValue,
                                                           imageinfo.IsSaturationChecked,
                                                           imageinfo.SaturationThreshold,
                                                           imageinfo.MixChannel.IsInvertChecked);
                }
                else
                {
                    ImageProcessing.Scale_16u8u_C1(pSrcimage, nWidth, nHeight, nSrcStride, pixelFormat, pDstImageData, nDstStride,
                                                   nBlackValue, nWhiteValue, dGammaValue,
                                                   imageinfo.IsSaturationChecked,
                                                   imageinfo.SaturationThreshold,
                                                   imageinfo.MixChannel.IsInvertChecked);
                ...
                ImageProcessing.Scale_16u8u_C1_Indexed(pSrcimage, nWidth, nHeight, nSrcStride, pixelFormat,
                                       pDstImageData, nDstStride,
                                       nBlackValue, nWhiteValue, dGammaValue,
                                       imageinfo.MaxPixelValue + 1,
                                       imageinfo.IsSaturationChecked,
                                       imageinfo.SaturationThreshold,
                                       imageinfo.MixChannel.IsInvertChecked,
                                       imageinfo.DynamicBit);
            }
        }
        ...
        public static unsafe WriteableBitmap PasteMarkerImage(WriteableBitmap chemiImage, WriteableBitmap blobsImage, WriteableBitmap clippedImage, Rect clippedRect)
        {
            ...
            histoMarkerMaxIndex = (int)((double)histoMarkerMaxIndex * ((double)65535.0 / (double)nBins));
        ...
        public static unsafe void FastClear(ref WriteableBitmap srcimage)
        {
            if (srcimage == null) { return; }

            var pixelWidth = srcimage.PixelWidth;
            var pixelHeight = srcimage.PixelHeight;
            int bytesPerPixel = (srcimage.Format.BitsPerPixel + 7) / 8;
            byte[] blankImage = new byte[pixelHeight * pixelWidth * bytesPerPixel];

            fixed (byte* bPtr = blankImage)
            {
                CopyMemory(srcimage.BackBuffer, (IntPtr)bPtr, blankImage.Length);
            }
        }
    }
}
