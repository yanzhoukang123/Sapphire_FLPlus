...
namespace Azure.Image.Processing
{
    ...
    public sealed class ColorSpaceHelper
    {
        ...
        #region Convert 4-channel to 3-channel
        public static RGB Convert4ChanneltoRGB(int red, int green, int blue, int gray)
        {
        ...
        public static RGB Convert4ChanneltoRGB(int red, int green, int blue, int gray, int nChannel, bool bIsGrayInverted)
        {
            int max = Math.Max(red, Math.Max(green, blue));
            //int min = Math.Min(red, Math.Min(green, blue));
            if (nChannel != 4)
            {
                // single channel contrast
                //
                if (nChannel == 3) // gray channel selected
                {
                    red = gray;
                    green = gray;
                    blue = gray;
            ...
            {
                // Multi-channel contrast
                //
                if (gray > max && !bIsGrayInverted)
                {
                    // display as gray
                    red = gray;
                    green = gray;
                ...
                else if (gray < max && bIsGrayInverted)
                {
                    // gray channel is inverted - assume chemi + marker
                    red = gray;
                    green = gray;
                    blue = gray;
                }
        ...
        public static CMYK NormalizeCMYK(double c, double m, double y, double k)
        {
            c = (double)(c / 255.0);
            m = (double)(m / 255.0);
            y = (double)(y / 255.0);
        ...
        /// <summary>
        /// Converts non-normalized CMYK values to RGB.
        /// Display CMYK as RGB + Gray (non-normalized CMYK to RGB -> RGB to CMYK -> CMYK to RGB).
        /// </summary>
        /// <param name="c"></param>
        /// <param name="m"></param>
        /// <param name="y"></param>
        ...
        public static RGB CMYK2RGB(double c, double m, double y, double k)
        {
            ...
            CMYK cmyk = RGBtoCMYK((int)c, (int)m, (int)y);
            //CMYK values from 0 to 1
            gray = (double)(k / 255.0);

            if (gray <= Math.Max(Math.Max(cmyk.Cyan, cmyk.Magenta), cmyk.Yellow))
            {
                //rgb
                red = Convert.ToInt32((1 - Math.Min(1, cmyk.Cyan * (1 - cmyk.Black) + cmyk.Black)) * 255);
                green = Convert.ToInt32((1 - Math.Min(1, cmyk.Magenta * (1 - cmyk.Black) + cmyk.Black)) * 255);
                blue = Convert.ToInt32((1 - Math.Min(1, cmyk.Yellow * (1 - cmyk.Black) + cmyk.Black)) * 255);
            }
            else
            {
                //gray
            ...
            return new RGB(Convert.ToInt32(red), Convert.ToInt32(green), Convert.ToInt32(blue));
        }

        public static CMYK RGBKtoCMYK(int red, int green, int blue, int black)
        {
            //cmyk value to 0 to 1
            double c = (double)(255 - red) / 255;
            double m = (double)(255 - green) / 255;
            double y = (double)(255 - blue) / 255;
            double k = (double)(255 - black) / 255;
